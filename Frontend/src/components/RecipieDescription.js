...
const RecipieDescription = ({meal}) => {
    ...
    const toggleBookmark = async () => {
        if ((userid == null) ||(userid=='null')) {
            ...
            try {
                const response = await fetch(`http://192.168.104.156:3000/recipie/bookmark`, {
                    method: 'POST',
    ...
    const checkIfBookmarked = async () => {
        try {
            const response = await fetch(`http://192.168.104.156:3000/recipie/isBookmarked`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ idMeal: meal.idMeal, userid })
            });
        ...
        <View style={{ flex: 1, paddingTop: hp(4) }}>
            <View style={{flexDirection:'row'}}>
                ...
                <Animated.View entering={FadeInDown.duration(700).springify().damping(12)} style={{ marginLeft: wp(3) }}>
                    ...
                    style={{marginLeft:wp(12),backgroundColor:'white',
